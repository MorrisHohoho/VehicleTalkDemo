...
testing_files_path = dname + "/baseline"
data = ORIGINAL_DATA

def generate_original_block(df):
...
def original_decode(block):
    received_str = ''
    for i in block:
        received_str += chr(i)
    if received_str == data:
        return(len(data),0)
    else:
        return (0,0)
...
if __name__ == "__main__":
    ...
    for file in findAllFile(testing_files_path):
        ...
        # Get original block
        for block in generate_original_block(df):
            this_block = block[0]
            this_latency = block[1]

            # original decode
            ret = original_decode(this_block)
