...
void VLC_transmitter_init(const int tx_pin,const int baud_rate,const int data_length)
{
    ...
    VLC_data_length=data_length;
    VLC_frame_length=VLC_data_length+1; //data + 1 byte PSN
...
{
    int frame_counts = strlen(data)/VLC_data_length;
	if(strlen(data)%VLC_data_length != 0)
	{
		frame_counts++;
	}
	...
	{
		encode(data+i*VLC_data_length,i,tx_buf);
