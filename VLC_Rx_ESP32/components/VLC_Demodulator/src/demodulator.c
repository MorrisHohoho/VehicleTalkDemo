...
void VLC_demodulator_config(const uint16_t rx_pin, const int baud_rate, const uint16_t frame_length){
    ...
    // 2. set the pin for UART
    ESP_ERROR_CHECK(uart_set_pin(UART_NUM_2,UART_PIN_NO_CHANGE,rx_pin,UART_PIN_NO_CHANGE,UART_PIN_NO_CHANGE));
...
void demodulator_receive(uint8_t *buffer)
{
    ...
    gpio_set_intr_type(VLC_rx1_pin,GPIO_INTR_NEGEDGE);
    gpio_intr_enable(VLC_rx1_pin);
...
static void IRAM_ATTR gpio_isr_handler_tim(void *arg)
{
    ...
    switch(header_clock_get_state())
    {
        case false:
        {
            gpio_intr_disable(VLC_rx1_pin);
            header_clock_start();
            gpio_set_intr_type(VLC_rx1_pin,GPIO_INTR_POSEDGE);
            gpio_intr_enable(VLC_rx1_pin);
            break;
        }
        case true:
